...
# 換模型救不了、或**潤稿不值得等**的錯要直接往上拋，立刻退組裝版。
# 429／5xx／timeout 的「換模型拚到底」政策只存在於焦點段自己的鏈
# （focus_today._post_gemini_hardy，見 test_focus）——潤稿有組裝版可退，
# 這裡維持原本行為：快速放棄、不悄悄換掉使用者設定的模型。
polish._DEAD.clear()
for name, code in [("143 400 不換模型", 400), ("144 401 不換模型", 401),
                   ("145 429 不換模型（潤稿直接退組裝版）", 429)]:
    calls = []
...
# ---------------------------------------------------------------------------
# ⑯ 連線層失敗（read timeout、斷線）：潤稿**不**換模型，直接往上拋。
#
# 這是還原後的原本行為（使用者的要求：潤稿快速退組裝版、不共用焦點段
# 的拚到底容錯）。timeout／429／5xx 換模型的政策見 test_focus 的
# _post_gemini_hardy 測試。
# ---------------------------------------------------------------------------
seq3 = []
...
try:
    ...
    _hit_to = True
check("147 timeout 不換模型、直接往上拋（退組裝版）",
      _hit_to and len(seq3) == 1, f"{len(seq3)} 次呼叫")
polish._DEAD.clear()
