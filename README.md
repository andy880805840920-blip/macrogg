...
tools/                     # 稽核腳本（前兩支需要 playwright；缺了會自動跳過）
  audit_layout.py          # 五個寬度量 DOM：溢出、觸控目標、卡片高度差、裁切
...
對照基準是**資料版本**不是執行時間。排程每天跑、資料一個月出一次，
「對照 08-11 05:01」會讓人以為核心 CPI 在一小時內掉了半個百分點。
資料月份沒變時直接講「自 7 月就業報告以來沒有新資料發布」，不列一堆
因為重跑而產生的微小差異。
...
顯示的單位是**交易**不是申報。同一筆公開發行通常會產生兩份申報
（424B 定價當日、8-K 2.03 四個營業日內），不合併的話畫面上同一筆債會出現兩次、
金額合計直接翻倍——而那個合計是這一段唯一有交易意涵的數字。
合併規則：同一家公司、金額相同、日期相差十天以內視為同一筆。

金額從說明書封面解析，解析不到就標「金額待確認」並附原文連結——
**不用推估值填補，也不計入合計**。一個錯的發債金額比沒有金額糟得多。
