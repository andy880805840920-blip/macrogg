...
tests/                     # 純 stdlib，直接 python tests/xxx.py 就能跑
  ...
  test_edgar_filings.py    # EDGAR 申報解析的例外路徑
tools/                     # 稽核腳本（需要 playwright；缺了會自動跳過）
...
**季報之外另外追發債申報**。10-Q 是季末後約 45 天才申報，所以一筆新發債
最久要等 135 天才會進到季報數字裡。但發債本身當天就要申報：424B2／424B5
是債券發行說明書本身，8-K 項目 2.03 是「產生直接財務義務」（四個營業日內）。
系統從 `data.sec.gov/submissions/` 抓近 120 天的這幾種申報，在科技巨頭卡的
最上方標示「已有 N 筆發債申報，尚未反映在下方的季報數字裡」，並試著從
說明書封面解析金額（解析不到就留空，不用推估值填補）。

這些事件**刻意不計入供給壓力分數**：分數維持由經審核的 XBRL 數字決定，
