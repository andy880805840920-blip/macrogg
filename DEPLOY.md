...
**⚠️ 有兩個資料夾絕對不要蓋上去：`output/` 與 `state/`。**

zip 裡的這兩個資料夾是**離線示範資料**（頁面上會有「離線示範模式」的橘色警示條）。
蓋上去的話：

- `output/` → Netlify 會立刻部署那份假資料，網站在下一次排程跑完前都是示範值
- `state/` → 下一次執行會拿**真實資料**跟**示範資料**比，
  「跟上期比，什麼變了」會生出一整頁不存在的變化

repo 裡原本的 `output/` 是上一次真實執行的產出，留著就好——
新程式推上去會觸發 Actions，它會用真實資料重新產生一份蓋掉。

**步驟（全程用網頁）**

1. 解壓縮 zip，打開 `labordashboard` 資料夾
2. **把裡面的 `output` 和 `state` 兩個資料夾刪掉**
3. 讓作業系統顯示隱藏檔案，否則 `.github` 與 `.gitignore` 會看不到也拖不到：
   - macOS Finder：按 `⌘` + `⇧` + `.`
   - Windows 檔案總管：檢視 → 顯示 → 勾「隱藏的項目」
4. GitHub repo 首頁 → **Add file** → **Upload files**
5. 把資料夾裡**剩下的全部**（含 `.github`、`.gitignore`）拖進去
6. 下方 **Commit changes**

上傳同名檔案是覆蓋，不會重複。上傳**不會刪除** repo 裡多出來的舊檔案，
不過這套專案沒有需要刪檔的情況。

**步驟 7：確認 workflow 真的換新了。** 這一步不能跳——
`.github` 是隱藏資料夾，最常見的失敗就是它根本沒被拖進去。

到 repo 的 `.github/workflows/update.yml`，確認檔案開頭附近有 `push:` 這一段。
沒有的話，直接在網頁上點那個檔案 → 鉛筆 → 把新版內容貼上去 → Commit。

**步驟 8：Actions 分頁應該會自動出現一筆新的執行。**
沒有的話手動觸發：**Actions** → **更新儀表板** → **Run workflow**。

**步驟 9：等綠勾（約 2–3 分鐘）→ Netlify 自動部署 → 重新整理網站。**
看頁面上方的「更新於」時間戳有沒有跟著變。

> 習慣用 git 的話，等價的做法是：解壓後
> `rsync -a --exclude output --exclude state 新資料夾/ 你的repo/`
> 然後 `git add -A && git commit -m "更新到新版" && git push`。
...
沒更新不會壞：市場預期沒填時會退回時間序列外推值並標明來源，SIFMA 發行量與 CPI 權重沒更新則沿用舊值、畫面上不另外提醒（所以要自己記得）。
