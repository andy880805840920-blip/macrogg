...
/* 結論卡底部通往情境頁的出口。做成一整塊可點的區域而不是一行文字連結：
   ...
   副標明講後面有什麼是首頁沒有的。 */
.v-cta{display:block;margin-top:14px;padding:12px 38px 12px 14px;
  ...
  font-weight:700;position:relative;line-height:1.5}
.v-cta span{display:block;font-size:12px;font-weight:400;color:var(--muted);
  margin-top:4px}
.v-cta::after{content:"";position:absolute;right:16px;top:calc(50% - 4px);
  width:7px;height:7px;border-top:1.5px solid var(--muted);
  border-right:1.5px solid var(--muted);transform:rotate(45deg)}
.v-cta:hover{border-color:var(--text-secondary)}
