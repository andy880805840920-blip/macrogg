...
.home-data-status{display:grid;grid-template-columns:1fr;gap:8px;background:transparent;
  border-style:dashed;box-shadow:none}
.home-data-status .home-zone-head{margin:0}
.home-data-status>div:not(.home-zone-head){display:flex;gap:9px;align-items:center}
.home-data-status>div:not(.home-zone-head) b{font-size:12px;color:var(--good)}
.home-data-status>div:not(.home-zone-head) span{font-size:12px;color:var(--muted)}
.home-data-status>p{font-size:12px;color:var(--muted);margin:0}
...
@media(min-width:760px){
  ...
  .home-data-status{grid-template-columns:auto auto minmax(0,1fr);align-items:center}
  .home-data-status .home-zone-head{min-width:150px}
...
/* 教學層：「這在看什麼？」。全站固定樣式與位置——讀者看熟了就知道
   ...
   一個是教你怎麼讀，一個是給你驗算。 */
.teach{margin:12px 0 0;border:1px dashed var(--grid);border-radius:9px;
  background:var(--surface-2)}
.teach>summary{cursor:pointer;list-style:none;padding:13px 12px;font-size:12.5px;
...
.teach>summary::before{content:"？";display:inline-block;width:17px;height:17px;
  line-height:17px;text-align:center;border-radius:50%;background:var(--surface);
  border:1px solid var(--grid);margin-right:7px;font-size:11px}
.teach-body{padding:2px 12px 10px;font-size:13px;line-height:1.8;
  color:var(--text-primary)}
