...
details.banner>summary::after{content:"";position:absolute;right:15px;
  top:calc(50% - 3px);width:7px;height:7px;border-right:1.5px solid var(--muted);
  border-bottom:1.5px solid var(--muted);transform:rotate(45deg);
  transform-origin:70% 70%;transition:transform .15s ease}
details.banner[open]>summary::after{transform:rotate(-135deg)}
...
.kpi .k-more>summary::before{content:"▸ "}
...
/* 手機上格子高度以內容為準（說明收合），min-height 若還留著桌機的高度，
...
/* 說明文只在「目前位置」那格常駐；其餘八格收成一顆「說明」，點了才展開
   （.sdd）。先前手機為了省高度把說明整批藏掉，現在收合式反而讓手機
   也看得到——想看哪格點哪格。summary 上下 12px 把觸控高撐過 40px。 */
.scell .sdesc{font-size:12px;color:var(--muted);margin-top:4px;line-height:1.5}
.sdd{margin:0;border:0;background:transparent}
.sdd>summary{cursor:pointer;list-style:none;display:inline-block;
  font-size:11px;font-weight:600;color:var(--muted);padding:12px 0}
.sdd>summary::-webkit-details-marker{display:none}
.sdd>summary::after{content:"▸";margin-left:3px;font-size:9px}
.sdd[open]>summary::after{content:"▾"}
.sdd .sdesc{margin-top:0}
...
.ax>summary::after{content:"";position:absolute;right:14px;top:calc(50% - 4px);
  width:7px;height:7px;border-right:1.5px solid var(--muted);
  border-bottom:1.5px solid var(--muted);transform:rotate(45deg);
  transform-origin:70% 70%;transition:transform .15s ease}
.ax[open]>summary::after{transform:rotate(-135deg)}
...
.home-brief-more[open]>summary::after{content:" －"}
...
.home-module>summary::after{content:"＋";position:absolute;right:3px;top:19px;
  color:var(--muted);font-size:17px;font-weight:400}
...
.focus-evidence>summary::after{content:" ＋";margin-left:3px}
.focus-evidence[open]>summary::after{content:" －"}
...
.light .l-more>summary::after{content:"▾";margin-left:4px}
.light .l-more[open]>summary::after{content:"▴"}
